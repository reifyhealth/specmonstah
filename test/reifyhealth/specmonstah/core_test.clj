    ...
    (walk/postwalk (fn [x]
                     (if-let [prefix (and (keyword? x) (matches? x))]
                       (do (swap! counter update prefix inc)
                           (keyword (str (name prefix) "-" (prefix @counter))))
                       x))
                   xs)))
    ...
    (is (= (->> (#'sm/bind-term-relations binding-template-relations [::site-user {:site-id :s1
           ...
           [::site-user {:site-id :s1
                         :site-user-tag-id [:site-tag-0 {:site-id :s1
                                                         :site-foo-id :sf1} {}]} nil]))
    (is (= (->> (#'sm/bind-term-relations binding-template-relations [::site-user {[::site-foo :id] :sf1}])
                (rename-generated-keys [:site-tag]))
           [::site-user {:site-user-tag-id [:site-tag-0 {:site-foo-id :sf1} {}]} nil]))))
